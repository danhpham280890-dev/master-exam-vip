...
app.openQuickEdit = function(id) {
    ...
    if (item.type === 'mcq' && item.options) {
        if (optArea) {
            ...
            // THÊM: Load số cột vào dropdown
            let colSelect = document.getElementById('edit-mcq-cols');
            if (colSelect) colSelect.value = item.columns || "auto";
        }
    }
...
app.updatePreview = function() {
    ...
    if (optArea && optArea.style.display !== 'none') {
        ...
        // Đọc số cột để đổi Live Preview luôn
        let colVal = document.getElementById('edit-mcq-cols') ? document.getElementById('edit-mcq-cols').value : "2";
        let gridCol = "1fr 1fr"; // default hiển thị 2
        if (colVal === "1") gridCol = "1fr";
        else if (colVal === "4") gridCol = "1fr 1fr 1fr 1fr";

        previewHTML += `<div style="display:grid; grid-template-columns: ${gridCol}; gap:10px; margin-top: 10px;">
...
app.saveQuickEdit = function() {
    ...
    if (item.type === 'mcq' && item.options) {
		...
		// THÊM: Lưu số cột vô item
        let colSelect = document.getElementById('edit-mcq-cols');
        if (colSelect) {
            if (colSelect.value === "auto") {
                delete item.columns; // Trả về tự động
            } else {
                item.columns = parseInt(colSelect.value); // Ép cứng số cột
            }
        }
    } 
