...
app.importFromCloud = async function() {
    ...
    // Ưu tiên lấy API từ localStorage, nếu trống thì dùng API mặc định
    ...
    try {
        ...
        // 3. Bắt lỗi từ SYSTEM_LOG trả về
        if (result.status === "error") {
            if (result.message.includes("Từ chối truy cập")) {
                throw new Error(`Email ${window.userEmail} không có trong SYSTEM_LOG hoặc đã bị khóa!`);
            } else {
...
app.importFastJSON = function() {
    ...
    if (!jsonText) {
        alert("⚠️ Bồ chưa dán mã JSON vào kìa!");
        return;
    }
    ...
    try {
        ...
        // Đề phòng AI xuất ra 1 object đơn thay vì mảng (Array)
        if (!Array.isArray(parsedData)) {
            parsedData = [parsedData]; 
        }
        
        // Cấp ID ảo thần tốc nếu AI quên cấp (để hệ thống không báo lỗi)
        parsedData.forEach((item, idx) => {
            if (!item._id) item._id = "q_ai_" + Date.now() + "_" + idx;
        });

        // Đẩy thẳng vào não bộ Master Exam
        ...
        console.error(e);
        alert("❌ Lỗi cấu trúc JSON! Bồ check lại xem AI có làm rớt dấu phẩy hay ngoặc kép nào không nha.\nLỗi chi tiết: " + e.message);
    }
};
